...
if(!idata) console.log("Check", shows.items[d_id][cnt]);
						...
						var utime=time;
try {
	console.log('Show:',name," ",logDate," ",shows.items[d_id][cnt].id);
} catch(e){}
						...
						if(shows.items[d_id][cnt].show.id==0 || shows.items[d_id][cnt].show.id=='special'){
							...
							} else if(window.inticketsToYandex && window.inticketsToYandex[shows.items[d_id][cnt].id]){
								buf1+='<div class="show"><span class="showtime">'+utime+'</span><span class="showname">'+uname+'</span>'+
								'<y:ticket data-session-id="'+window.inticketsToYandex[shows.items[d_id][cnt].id]+'" data-template="yandex-button"></y:ticket>'+
								'</div>';
							} else {
    ...
    for(var cnt=0,m=list.length;cnt<m;cnt++)
      list[cnt].addEventListener('click',_blockEvent,true);
		if(window.onCalendarPage) window.onCalendarPage();
		return date;
	}
